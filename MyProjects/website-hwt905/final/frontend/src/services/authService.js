...
authAPI.interceptors.response.use(
  ...
  (error) => {
    ...
    if (error.response?.status === 401) {
      ...
      // No response from server - connection issue
      if (error.code === 'ECONNREFUSED') {
        error.message = `Không thể kết nối server authentication. Vui lòng kiểm tra:
- Backend server có đang chạy không? (http://localhost:5000)
- API URL có đúng không? (${BASE_URL})`;
      } else if (error.code === 'ETIMEDOUT' || error.code === 'ECONNABORTED') {
        error.message = 'Kết nối server authentication bị timeout. Vui lòng thử lại sau.';
      } else if (error.message && error.message.includes('Network Error')) {
        error.message = `Lỗi mạng khi kết nối server authentication. Vui lòng kiểm tra:
- Backend server có đang chạy không? (http://localhost:5000)
