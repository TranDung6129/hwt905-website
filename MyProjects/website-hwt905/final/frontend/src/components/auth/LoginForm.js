  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (error.response?.status === 401) {
        setErrors({
          general: 'Tên đăng nhập/email hoặc mật khẩu không đúng'
        });
  ...
  const handleDemoLogin = async () => {
    ...
    setTimeout(async () => {
      try {
        ...
        let errorMessage = 'Đăng nhập demo thất bại';
        
        if (error.message) {
          if (error.message.includes('Không thể kết nối')) {
            errorMessage = error.message;
          } else {
            errorMessage = `Đăng nhập demo thất bại: ${error.message}`;
          }
        } else {
          errorMessage = 'Đăng nhập demo thất bại: Không thể kết nối server authentication';
        }
  ...
  return (
    <div className="login-form">
      ...
      <button
        ...
        disabled={isSubmitting}
      >
        Đăng nhập với tài khoản demo (admin/admin123)
      </button>
